...
bl_UnixSort(void *space, char *filename, const char *fieldstring, const char delim) {
  ...
  NFO("executing command '%s'\n", cmd);
  
  system(cmd);
  
  bl_rm(space, filename);
  int val = rename(tempfilename, filename);
  
  if(val == 0) {
    NFO("renamed file '%s' successfully to '%s'\n.", tempfilename, filename);
  } else {
    char* errstring = strerror(errno);
    NFO("renaming file '%s' to '%s' unsuccessful: %s\n.", tempfilename, filename, errstring);
    NFO("trying to move '%s' file instead\n", tempfilename);
 
    cmd2 = calloc(strlen(mv) + strlen(tempfilename) + strlen(filename) + 15, 1); 
    sprintf(cmd2, "%s %s %s", mv, tempfilename, filename);
    NFO("executing command '%s'\n", cmd2);
    system(cmd2);
