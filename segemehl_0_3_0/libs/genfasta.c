...
main(int argc, char **argv) {
  ...
  if(dbfilename) {
    ...
    sequence = bl_fastaGetSequence(fasta, 0);
    reflen = bl_fastaGetSequenceLength(fasta, 0);
  }
...
*/
  ...
  if(fiveprime) {
    ...
    fprintf(stderr,"5prime adapter: %s (%d)\n", fiveprime, fiveprimelen);
  }
  if(threeprime) {
    threeprimelen = strlen(threeprime);
    fprintf(stderr,"3prime adapter: %s (%d)\n", threeprime, threeprimelen);
  }


  if(sequence && !matefilename && !split && !bisulfite) {
  ...
  if(sequence && splice && !matefilename && !bisulfite) {
  ...
  if(sequence && split && !matefilename && !bisulfite) {
    fprintf(stderr, "reflen: %d; maxreadlen %d; '%s'-'%s'\n", reflen, maxreadlen, fiveprime, threeprime);
  ...
  if(sequence && matefilename && !bisulfite) {
