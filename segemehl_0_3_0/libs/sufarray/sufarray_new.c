    ...
    if (j > 0) {
     k = arr->suftab[j-1];
      ...
      while (arr->seq->sequences[i+l] == arr->seq->sequences[k+l] 
                      && arr->seq->sequences[i+l] != (char)126 //commented out
                      && arr->seq->sequences[k+l] != (char)126 //commented out
          ){ 
        l++;
      }
...
checklcps(Suffixarray *arr) {
  ...
  for(i=0; i < arr->numofsuffixes; i++) {
    if (lcp(arr,i) > 0) {
      ...
      for(j=0; j < lcp(arr,i); j++) 
        assert(arr->seq->sequences[s+j] == arr->seq->sequences[t+j]);
