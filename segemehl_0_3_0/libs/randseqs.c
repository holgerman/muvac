    ...
    char *three, Uint threelen, Uint polyAlen)
{
  ...
  for (i=0; i < n; i++) {
    ...
    while (start > seqlen-(maxdist+2*maxlen+1) || strncmp(&sequence[start],"NNN\0", 3)==0) start = RANDINT(seqlen);
    ...
    char *three, Uint threelen, Uint polyAlen)
{
  ...
  *editstring, startchr='@', chr, errchr=0;
  ...
  buffer = ALLOCMEMORY(space, NULL, char, noofsplits*maxspltlen+3);
  quality =  ALLOCMEMORY(space, NULL, char, noofsplits*maxspltlen+3);
  editstring = ALLOCMEMORY(space, NULL, char, 40*(noofsplits*maxspltlen+polyAlen+threelen+fivelen));
  ...
  for (i=0; i < n; i++) {
    start=seqlen;
    memset(buffer, 0, noofsplits*maxspltlen+3);
    memset(quality, 0, noofsplits*maxspltlen+3);
    ...
    while (start > seqlen-(2*maxspltlen) || strncmp(&sequence[start],"NNN\0", 3)==0) start = RANDINT(seqlen);
   ...
   if(RANDUNIT > fiveacc) {
    ...
    /*dump results*/
    fprintf(dev, "%s (r: %f)\n", editstring, (double) readerrcnt/(spltlen+spltlen2+1.0));
