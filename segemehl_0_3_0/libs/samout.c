...
void sam_setSplitStrand(samrec_t *samrec, char strand) {
  
  if(strand == 1) {
    //positive strand
    sam_addTag(samrec, "XS:A:%c", '+');
  } else if(strand == 2) {
    //negative strand
    sam_addTag(samrec, "XS:A:%c", '-');
  } else if(strand == 3) { 
...
samlist_t *sam_getSamList (fasta_t *reads, Uint id, mapping_t *l, 
  ...
  for(i=0; i < nfrags; i++) {
    ...
    if(nfo->split) {
      ...
      sam_setSplitStrand(&samlist->recs[i], strand);
    }
    ...
    if(nfo->mappingqual == 0) { 
      if(isChimeric[!isQuery]) {
        sam_setMapq(&samlist->recs[i], MAX(l->mapqual, minqual));
      } else { 
